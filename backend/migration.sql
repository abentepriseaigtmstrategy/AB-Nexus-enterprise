-- ════════════════════════════════════════════════════════════════════
-- AB Nexus Enterprise — Live Database Migration
-- Run once against your live D1 database:
-- wrangler d1 execute ab-nexus-db --remote --file=migration.sql
--
-- ALL statements are safe to run even if some columns already exist.
-- If any line errors "duplicate column name" — that column exists, skip it.
-- ════════════════════════════════════════════════════════════════════

-- STEP 1: Add missing columns to claims table
ALTER TABLE claims ADD COLUMN current_stage TEXT DEFAULT 'intake';
ALTER TABLE claims ADD COLUMN fsr_ready INTEGER DEFAULT 0;
ALTER TABLE claims ADD COLUMN insured_address TEXT;
ALTER TABLE claims ADD COLUMN insured_phone TEXT;
ALTER TABLE claims ADD COLUMN insured_email TEXT;
ALTER TABLE claims ADD COLUMN insurer_name TEXT;
ALTER TABLE claims ADD COLUMN insurer_code TEXT;
ALTER TABLE claims ADD COLUMN revision_source TEXT;
ALTER TABLE claims ADD COLUMN revision_reason TEXT;
ALTER TABLE claims ADD COLUMN revision_from_version TEXT;

-- STEP 2: Create survey_reports_pipeline if it doesn't exist
...
-- STEP 3: Create report_audit table (needed for FSR submission tracking)
...
-- STEP 4: Create stage_deadlines table
...
-- STEP 5: Create stage_snapshots table
...
-- STEP 6: Create fsr_calculations table
...
-- STEP 7: Create vault_entries table
...
-- STEP 8: Create document_checklist table
...
-- STEP 9: Create sessions table if missing
